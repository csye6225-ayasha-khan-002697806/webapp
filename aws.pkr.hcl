...
variable "subnet_id" { type    = string default = "subnet-08b7aaccb1c0bf34b"
}
...
aws_polling { delay_seconds = 120 max_attempts  = 50 }
...
build   {  sources = [  "source.amazon-ebs.my-ami",  ]

provisioner "shell" {
 environment_vars = [ "DEBIAN_FRONTEND=noninteractive", "CHECKPOINT_DISABLE=1" ] 
inline = [  "sudo apt-get update", "sudo apt-get upgrade -y", "sudo apt-get install nginx -y", "sudo apt-get clean",  ]
}
}
